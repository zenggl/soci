...
protected:
    odbc_standard_type_backend_base(odbc_statement_backend &st) : statement_(st) {}
...
struct odbc_standard_into_type_backend : details::standard_into_type_backend, private odbc_standard_type_backend_base {
    odbc_standard_into_type_backend(odbc_statement_backend &st) : odbc_standard_type_backend_base(st), buf_(0) {}
...
struct odbc_vector_into_type_backend : details::vector_into_type_backend, private odbc_standard_type_backend_base {
    odbc_vector_into_type_backend(odbc_statement_backend &st)
        : odbc_standard_type_backend_base(st), data_(NULL), buf_(NULL), position_(0) {}
...
struct odbc_standard_use_type_backend : details::standard_use_type_backend, private odbc_standard_type_backend_base {
    odbc_standard_use_type_backend(odbc_statement_backend &st)
        : odbc_standard_type_backend_base(st), position_(-1), data_(0), buf_(0), indHolder_(0) {}
...
private:
    ...
    void copy_from_string(std::string const &s, SQLLEN &size, SQLSMALLINT &sqlType, SQLSMALLINT &cType);
};

struct odbc_vector_use_type_backend : details::vector_use_type_backend, private odbc_standard_type_backend_base {
    odbc_vector_use_type_backend(odbc_statement_backend &st)
        : odbc_standard_type_backend_base(st), data_(NULL), buf_(NULL) {}
...
struct odbc_session_backend : details::session_backend {
    ...
    bool get_last_insert_id(session &s, std::string const &table, long long &value) override;
...
public:
    ...
    error_category get_error_category() const override {
        ...
        if ((s[0] == '0' && s[1] == '8') || strcmp(s, "HYT01") == 0) return connection_error;

        if (strcmp(s, "23000") == 0 || strcmp(s, "40002") == 0 || strcmp(s, "44000") == 0) return constraint_violation;

        if (strcmp(s, "HY014") == 0) return system_error;

        return unknown;
    }

    SQLCHAR const *odbc_error_code() const { return sqlstate_; }
    SQLINTEGER native_error_code() const { return sqlcode_; }
    SQLCHAR const *odbc_error_message() const { return message_; }
...
private:
    std::string interpret_odbc_error(SQLSMALLINT htype, SQLHANDLE hndl, std::string const &msg) {
        ...
        switch (SQLGetDiagRecA(htype, hndl, i, sqlstate_, &sqlcode_, message_, SQL_MAX_MESSAGE_LENGTH + 1, &length)) {
            case SQL_SUCCESS:
...
inline bool odbc_standard_type_backend_base::use_string_for_bigint() const {
    ...
    // them to SQL_NUMERIC.
    return statement_.session_.get_database_product() == odbc_session_backend::prod_oracle;
}
...
inline bool odbc_standard_type_backend_base::requires_noncompliant_32bit_sqllen() const {
    ...
    return statement_.session_.get_database_product() == odbc_session_backend::prod_db2;
#else
    return false;
#endif
}
...
inline bool odbc_standard_type_backend_base::supports_negative_tinyint() const {
    ...
    return statement_.session_.get_database_product() != odbc_session_backend::prod_mssql;
}
...
struct odbc_backend_factory : backend_factory {
    ...
    odbc_session_backend *make_session(connection_parameters const &parameters) const override;
};
