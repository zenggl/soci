...
private:
    ...
    ~mysql_library() { mysql_library_end(); }

public:
    // This function only exists for its side effect of creating an instance of
    // this class.
    static void ensure_initialized() { static mysql_library ins; }
};

void skip_white(std::string::const_iterator *i, std::string::const_iterator const &end, bool endok) {
    for (;;) {
        if (*i == end) {
            if (endok) {
                return;
...
std::string param_name(std::string::const_iterator *i, std::string::const_iterator const &end) {
    std::string val("");
    for (;;) {
...
string param_value(string::const_iterator *i, string::const_iterator const &end) {
...
bool valid_uint(const string &s) {
    ...
    if (errno != 0 || n == 0 || n > UINT_MAX) return false;
    if (*tail != '\0') return false;
    return true;
}

void parse_connect_string(const string &connectString, string *host, bool *host_p, string *user, bool *user_p,
                          string *password, bool *password_p, string *db, bool *db_p, string *unix_socket,
                          bool *unix_socket_p, int *port, bool *port_p, string *ssl_ca, bool *ssl_ca_p,
                          string *ssl_cert, bool *ssl_cert_p, string *ssl_key, bool *ssl_key_p, int *local_infile,
                          bool *local_infile_p, string *charset, bool *charset_p, bool *reconnect_p,
                          unsigned int *connect_timeout, bool *connect_timeout_p, unsigned int *read_timeout,
                          bool *read_timeout_p, unsigned int *write_timeout, bool *write_timeout_p,
    ...
    *ssl_mode_p = false;
    string err = "Malformed connection string.";
    string::const_iterator i = connectString.begin(), end = connectString.end();
    while (i != end) {
        ...
        if (par == "port" && !*port_p) {
            ...
            *charset_p = true;
            ...
            if (val != "1") throw soci_error("\"reconnect\" option may only be set to 1");

            *reconnect_p = true;
        } else if (par == "connect_timeout" && !*connect_timeout_p) {
            if (!valid_uint(val)) throw soci_error(err);
            char *endp;
            ...
            *write_timeout_p = true;
            ...
            if (val == "DISABLED")
                ...
                *ssl_mode = SSL_MODE_VERIFY_IDENTITY;
                ...
                throw soci_error("\"ssl_mode\" setting is invalid");
            *ssl_mode_p = true;
#else
...
mysql_session_backend::mysql_session_backend(connection_parameters const &parameters) {
    mysql_library::ensure_initialized();

    string host, user, password, db, unix_socket, ssl_ca, ssl_cert, ssl_key, charset;
    int port, local_infile;
    unsigned int connect_timeout, read_timeout, write_timeout, ssl_mode;
    bool host_p, user_p, password_p, db_p, unix_socket_p, port_p, ssl_ca_p, ssl_cert_p, ssl_key_p, local_infile_p,
        charset_p, reconnect_p, connect_timeout_p, read_timeout_p, write_timeout_p, ssl_mode_p;
    parse_connect_string(parameters.get_connect_string(), &host, &host_p, &user, &user_p, &password, &password_p, &db,
                         &db_p, &unix_socket, &unix_socket_p, &port, &port_p, &ssl_ca, &ssl_ca_p, &ssl_cert,
                         &ssl_cert_p, &ssl_key, &ssl_key_p, &local_infile, &local_infile_p, &charset, &charset_p,
                         &reconnect_p, &connect_timeout, &connect_timeout_p, &read_timeout, &read_timeout_p,
                         &write_timeout, &write_timeout_p, &ssl_mode, &ssl_mode_p);
    ...
    if (local_infile_p && local_infile == 1) {
        if (0 != mysql_options(conn_, MYSQL_OPT_LOCAL_INFILE, NULL)) {
            clean_up();
            throw soci_error("mysql_options() failed when trying to set local-infile.");
        }
    }
...
#endif
    if (mysql_real_connect(conn_, host_p ? host.c_str() : NULL, user_p ? user.c_str() : NULL,
                           password_p ? password.c_str() : NULL, db_p ? db.c_str() : NULL, port_p ? port : 0,
...
mysql_session_backend::~mysql_session_backend() { clean_up(); }
...
bool mysql_session_backend::is_connected() { return mysql_ping(conn_) == 0; }

void mysql_session_backend::begin() { hard_exec(conn_, "BEGIN"); }

void mysql_session_backend::commit() { hard_exec(conn_, "COMMIT"); }

void mysql_session_backend::rollback() { hard_exec(conn_, "ROLLBACK"); }

bool mysql_session_backend::get_last_insert_id(session & /* s */, std::string const & /* table */, long long &value) {
 ...
 * @param offset        返回 偏移量
 */
std::string mysql_session_backend::get_page_query_sql(const std::string &select_sql, int page_no, int page_size,
...
mysql_rowid_backend *mysql_session_backend::make_rowid_backend() { return new mysql_rowid_backend(*this); }

mysql_blob_backend *mysql_session_backend::make_blob_backend() { return new mysql_blob_backend(*this); }
