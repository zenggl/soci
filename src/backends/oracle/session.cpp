...
sb4 fo_callback(void * /* svchp */, void * /* envhp */, void *fo_ctx, ub4 /* fo_type */, ub4 fo_event) {
    oracle_session_backend *backend = static_cast<oracle_session_backend *>(fo_ctx);
...
oracle_session_backend::oracle_session_backend(std::string const &serviceName, std::string const &userName,
                                               std::string const &password, int mode, bool decimals_as_strings,
                                               int charset, int ncharset)
    : envhp_(NULL), srvhp_(NULL), errhp_(NULL), svchp_(NULL), usrhp_(NULL), decimals_as_strings_(decimals_as_strings) {
    ...
    res = OCIEnvNlsCreate(&envhp_, OCI_THREADED | OCI_ENV_NO_MUTEX, 0, 0, 0, 0, 0, 0, charset, ncharset);
    if (res != OCI_SUCCESS) {
        throw soci_error("Cannot create environment");
    }

    // create the server handle
    res = OCIHandleAlloc(envhp_, reinterpret_cast<dvoid **>(&srvhp_), OCI_HTYPE_SERVER, 0, 0);
    if (res != OCI_SUCCESS) {
        clean_up();
    ...
    res = OCIHandleAlloc(envhp_, reinterpret_cast<dvoid **>(&errhp_), OCI_HTYPE_ERROR, 0, 0);
    if (res != OCI_SUCCESS) {
        clean_up();
    ...
    if (charset != 0) {
        ...
        res = OCINlsCharSetConvert(envhp_, errhp_, charset, nlsService, serviceBufLen, defaultSourceCharSetId,
                                   serviceName.c_str(), serviceName.size(), &nlsServiceLen);
        if (res != OCI_SUCCESS) {
            std::string msg;
            int errNum;
        ...
        res = OCINlsCharSetConvert(envhp_, errhp_, charset, nlsUserName, authBufLen, defaultSourceCharSetId,
                                   userName.c_str(), userName.size(), &nlsUserNameLen);
        if (res != OCI_SUCCESS) {
            std::string msg;
            int errNum;
            get_error_details(res, errhp_, msg, errNum);
        ...
        res = OCINlsCharSetConvert(envhp_, errhp_, charset, nlsPassword, authBufLen, defaultSourceCharSetId,
                                   password.c_str(), password.size(), &nlsPasswordLen);
        if (res != OCI_SUCCESS) {
            std::string msg;
            int errNum;
            get_error_details(res, errhp_, msg, errNum);
    ...
    res = OCIServerAttach(srvhp_, errhp_, reinterpret_cast<text *>(nlsService), static_cast<sb4>(nlsServiceLen),
                          OCI_DEFAULT);
    if (res != OCI_SUCCESS) {
        std::string msg;
        int errNum;
    ...
    res = OCIAttrSet(srvhp_, static_cast<ub4>(OCI_HTYPE_SERVER), &fo, 0, static_cast<ub4>(OCI_ATTR_FOCBK), errhp_);
    if (res != OCI_SUCCESS) {
        std::string msg;
        int errNum;
        get_error_details(res, errhp_, msg, errNum);
    ...
    res = OCIHandleAlloc(envhp_, reinterpret_cast<dvoid **>(&svchp_), OCI_HTYPE_SVCCTX, 0, 0);
    if (res != OCI_SUCCESS) {
        clean_up();
    ...
    res = OCIAttrSet(svchp_, OCI_HTYPE_SVCCTX, srvhp_, 0, OCI_ATTR_SERVER, errhp_);
    if (res != OCI_SUCCESS) {
        std::string msg;
        int errNum;
        get_error_details(res, errhp_, msg, errNum);
    ...
    res = OCIHandleAlloc(envhp_, reinterpret_cast<dvoid **>(&usrhp_), OCI_HTYPE_SESSION, 0, 0);
    if (res != OCI_SUCCESS) {
        clean_up();
    ...
    if (userName.empty() && password.empty()) {
        ...
        res = OCIAttrSet(usrhp_, OCI_HTYPE_SESSION, reinterpret_cast<dvoid *>(nlsUserName),
        ...
        res = OCIAttrSet(usrhp_, OCI_HTYPE_SESSION, reinterpret_cast<dvoid *>(nlsPassword),
    ...
    // begin the session
    res = OCISessionBegin(svchp_, errhp_, usrhp_, credentialType, mode);
    ...
    res = OCIAttrSet(svchp_, OCI_HTYPE_SVCCTX, usrhp_, 0, OCI_ATTR_SESSION, errhp_);
    if (res != OCI_SUCCESS) {
        std::string msg;
        int errNum;
        get_error_details(res, errhp_, msg, errNum);
...
oracle_session_backend::~oracle_session_backend() { clean_up(); }

bool oracle_session_backend::is_connected() { return OCIPing(svchp_, errhp_, OCI_DEFAULT) == OCI_SUCCESS; }
...
oracle_rowid_backend *oracle_session_backend::make_rowid_backend() { return new oracle_rowid_backend(*this); }

oracle_blob_backend *oracle_session_backend::make_blob_backend() { return new oracle_blob_backend(*this); }

bool oracle_session_backend::get_next_sequence_value(session &s, std::string const &sequence, long long &value) {
 ...
 * @param offset        返回 偏移量
 */
std::string oracle_session_backend::get_page_query_sql(const std::string &select_sql, int page_no, int page_size,
    ...
    ss << "select * from ";
    ss << " ( ";
    ss << " select socit.*, rownum ";
    ss << " from ( \n" << select_sql << "\n ) socit ";
    ss << " where rownum < :limit )";
    ss << " where rownum >= :offset";
