...
odbc_session_backend::odbc_session_backend(connection_parameters const &parameters)
    ...
    if (is_odbc_error(rc)) {
        throw odbc_soci_error(SQL_HANDLE_DBC, hdbc_, "allocating connection handle");
    }
    ...
    if (parameters.is_option_on(option_reconnect)) {
        ...
        if (parameters.get_option(odbc_option_driver_complete, completionString)) {
            ...
            if (std::sscanf(completionString.c_str(), "%u", &completion) != 1) {
                throw soci_error("Invalid non-numeric driver completion option value \"" + completionString + "\".");
            }
        }
    }

#ifdef _WIN32
    if (completion != SQL_DRIVER_NOPROMPT) hwnd_for_prompt = ::GetDesktopWindow();
#endif  // _WIN32
    ...
    for (;;) {
        rc = SQLDriverConnect(hdbc_, hwnd_for_prompt, sqlchar_cast(connectString), (SQLSMALLINT)connectString.size(),
                              outConnString, 1024, &strLength, static_cast<SQLUSMALLINT>(completion));
        ...
        switch (rc) {
            ...
            default:
                ...
                if (memcmp(err.odbc_error_code(), "HY110", 6) == 0 && completion != SQL_DRIVER_NOPROMPT) {
                    errContext =
                        "while retrying to connect without prompting, as "
                ...
                if (!errContext.empty()) err.add_context(errContext);
...
void odbc_session_backend::configure_connection() {
    if (get_database_product() == prod_postgresql) {
        ...
        if (is_odbc_error(rc)) {
            throw odbc_soci_error(SQL_HANDLE_DBC, henv_, "getting PostgreSQL ODBC driver version");
        }
        ...
        if (std::sscanf(product_ver, "%u", &major_ver) != 1) {
            throw soci_error("DBMS version \"" + std::string(product_ver) + "\" in unrecognizable format.");
        }

        details::auto_statement<odbc_statement_backend> st(*this);

        std::string const q(major_ver >= 9 ? "SET extra_float_digits = 3" : "SET extra_float_digits = 2");
        rc = SQLExecDirect(st.hstmt_, sqlchar_cast(q), static_cast<SQLINTEGER>(q.size()));

        if (is_odbc_error(rc)) {
            throw odbc_soci_error(SQL_HANDLE_DBC, henv_, "setting extra_float_digits for PostgreSQL");
        }

        // This is extracted from pgapifunc.h header from psqlODBC driver.
        enum { SQL_ATTR_PGOPT_UNKNOWNSASLONGVARCHAR = 65544 };
...
odbc_session_backend::~odbc_session_backend() { clean_up(); }
...
bool odbc_session_backend::is_connected() {
    ...
    return !is_odbc_error(
        SQLTables(st.hstmt_, NULL, SQL_NTS, NULL, SQL_NTS, sqlchar_cast("bloordyblop"), SQL_NTS, NULL, SQL_NTS));
}

void odbc_session_backend::begin() {
    SQLRETURN rc = SQLSetConnectAttr(hdbc_, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_OFF, 0);
    if (is_odbc_error(rc)) {
...
bool odbc_session_backend::get_next_sequence_value(session &s, std::string const &sequence, long long &value) {
    std::string query;
...
bool odbc_session_backend::get_last_insert_id(session &s, std::string const &table, long long &value) {
    std::string query;
...
void odbc_session_backend::reset_transaction() {
    SQLRETURN rc = SQLSetConnectAttr(hdbc_, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_ON, 0);
    if (is_odbc_error(rc)) {
...
odbc_statement_backend *odbc_session_backend::make_statement_backend() { return new odbc_statement_backend(*this); }

odbc_rowid_backend *odbc_session_backend::make_rowid_backend() { return new odbc_rowid_backend(*this); }

odbc_blob_backend *odbc_session_backend::make_blob_backend() { return new odbc_blob_backend(*this); }

odbc_session_backend::database_product odbc_session_backend::get_database_product() const {
    // Cache the product type, it's not going to change during our life time.
    if (product_ != prod_uninitialized) return product_;
    ...
    if (is_odbc_error(rc)) {
        throw odbc_soci_error(SQL_HANDLE_DBC, henv_, "getting ODBC driver name");
    }
 ...
 * @param offset        返回 偏移量
 */
std::string odbc_session_backend::get_page_query_sql(const std::string &select_sql, int page_no, int page_size,
